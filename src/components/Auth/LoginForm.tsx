...
export function LoginForm({ onSwitchToRegister, onClose }: LoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        console.log('✅ LoginForm: Login bem-sucedido, fechando modal...');
        onClose();
        
        // Force page refresh to ensure proper state update
        setTimeout(() => {
          console.log('🔄 LoginForm: Forçando atualização da página...');
          window.location.reload();
        }, 100);
      } else {
        console.error('❌ LoginForm: Falha no login:', result.error);
        setError(result.error || 'Erro ao fazer login');
      }
    } catch (err) {
      console.error('❌ LoginForm: Erro durante o login:', err);
